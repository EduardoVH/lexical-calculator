...
function App() {
    ...
    function generateGraph(tree) {
        ...
        // Adjust the layout based on the graph size
        const graphSize = g.graph();
        const width = graphSize.width || 800; // Set default width if graph size is not available
        const height = graphSize.height || 600; // Set default height if graph size is not available
        ...
        svg.call(zoom.transform, d3.zoomIdentity.translate(width / 2, 20));
        svg.call(zoom);
    }
    ...
    return (
        <div className="calculator-container">
            <div className="calculator">
                <div
                    className="flex flex-col rounded-lg overflow-hidden bg-black bg-opacity-50 backdrop-filter backdrop-blur-xl">
                    <div className="display">{expression === "" ? "0" : expression}</div>
                    <div className="buttons">
                        ...
                        <button
                            className="py-5 bg-purple-400 bg-opacity-10 text-white font-bold text-xl focus:outline-none hover:bg-opacity-25 transition-all duration-200"></button>
                    </div>
                </div>
            </div>
                <div className="side-content">
                    <div>
                        {tokens.map((token, index) => (
                            <li className="pl-10 lexico" key={index}>
                                Digito [ {token.value} ] - Tipo: {token.type}
                            </li>
                        ))}
                    </div>
